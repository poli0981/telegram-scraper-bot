...
- Publishing others' private information, such as a physical or email
  address, **including their Telegram User ID**, without their explicit
  permission.
- Other conduct that could reasonably be considered inappropriate in a
  professional setting.

## Telegram User ID handling (project-specific)

This is a Telegram bot project. A user's `user_id` is sensitive personally
identifiable information in our context: it lets others target them on
Telegram and may correlate accounts across services.

- **Never** include another user's `user_id` in a public issue, PR,
  discussion, or screenshot.
- If a `user_id` must be shared (e.g. requesting addition to
  `ALLOWED_USER_IDS`), send it via direct message to the maintainer using
  one of the channels in [`AUTHORS.md`](AUTHORS.md) — never in a public
  channel.
- The `/whoami` command outputs *your own* ID. Do not paste its output into
  public threads.

Maintainers will redact public posts that contain other users' Telegram IDs
without consent.
...
in public spaces. Examples of representing the project include using an
...
reported to the project maintainers via any of the channels listed in
[`AUTHORS.md`](AUTHORS.md). All complaints will be reviewed and investigated
promptly and fairly.
